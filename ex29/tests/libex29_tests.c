...
int check_function(const char *func_to_run, const char *data, int expected) {
  ...
  check(func != NULL, "Did not find any %s function in the library %s: %s",
...
char *test_dlopen() {
  ...
  // if lib is not NULL, this will return the message, thus failing the test
  mu_assert(lib != NULL, "Failed to open the library to test");

  // NOTE: returning NULL means the test pass
  return NULL;
}
