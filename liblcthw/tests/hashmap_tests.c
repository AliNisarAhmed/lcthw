...
char *test_delete() {
  ...
  mu_assert(deleted == &expect1, "Should get test2");
  result = Hashmap_get(map, &test2);
  mu_assert(result == NULL, "should delete test2");

  deleted = (bstring)Hashmap_delete(map, &test3);
  mu_assert(deleted != NULL, "Got NULL on delete test3");
  mu_assert(deleted == &expect1, "Should get test3");
  result = Hashmap_get(map, &test3);
  mu_assert(result == NULL, "should delete test3");
