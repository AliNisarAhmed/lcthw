...
int list_files(glob_t *pglob) {
  ...
  check_mem(line);
  check(file, "Failed to open .logfind. Make sure that file exists in current "
              "directory");
...
int scan_file(const char *filename, int use_or, int search_len,
  ...
  // read each line of the file and search that line for contents
  while (fgets(line, MAX_LINE - 1, file) != NULL) {
...
int parse_args(int *use_or, int *argc, char **argv[]) {
  ...
  if (strcmp((*argv)[0], "-o") == 0) {
    *use_or = 1;
    (*argc)--; // skipping -o
    (*argv)++;
    check(*argc > 1, "You need to write words after -o");
  } else {
    use_or = 0;
  }
